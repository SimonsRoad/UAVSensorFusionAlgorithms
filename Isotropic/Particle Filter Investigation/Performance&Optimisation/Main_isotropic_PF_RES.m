function [x_state, x_t_vec, P_cov,time] = Main_isotropic_PF_RES(plotting, Q, R, x_t_vec, x_uav, psi_uav, P_ini,N)
...
for k=1:N_loops_fb                                                             %   Begin main 'for' loop: for all time steps
    ...
    %   Kalman filtering: EKF (UKF)
        ...
        if (((obs_check==1)&&(k==k_obs))||(re_run_bool==1))              	%   If intersections have begun & first time EKF (UKF) is run
            ...
            %%% and UKF
            [x_state(:,k),P_cov(:,:,k)]=PF_form_RES(x_vec_all(1,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,1),x_state_ini,P_cov_ini,F_KF,G_KF,Q_KF,R_KF,N);
