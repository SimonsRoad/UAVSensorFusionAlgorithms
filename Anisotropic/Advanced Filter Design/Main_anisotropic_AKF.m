...
for k=1:N_loops_fb                                                             %   Begin main 'for' loop: for all time steps
    ...
    %   Filtering
        ...
        if (((obs_check==1)&&(k==k_obs))||(re_run_bool==1))              	%   If intersections have begun & first time filter is run
            ...
            [x_state(:,k),P_cov(:,:,k),K_EKF_gain(:,k)]=AKF_form(x_vec_all(1,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,1),x_state_ini,P_cov_ini,F_KF,G_KF,Q_KF,R_KF,G_t_sim_sto(1,1),G_t_sim_sto(k,1));
